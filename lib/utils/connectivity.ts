...
export class ConnectivityChecker {
  static async validateWebSocketEndpoint(endpoint: string): Promise<boolean> {
    ...
    if (!wsRegex.test(endpoint)) {
      throw new PlaywrightConnectionError(`Invalid WebSocket endpoint format: ${endpoint}`);
    }
    return true;
  }
  ...
  static async createBrowserConnection(options: {
    ...
    try {
      if (options.wsEndpoint) {
        ...
        return await (playwrightChromium as any)[browserType].launch(options.launchOptions || {});
      }
    } catch (err) {
  ...
  static async establishConnection(browserWSEndpoint: string, options = {
    maxRetries: 3,
    timeoutMs: 30000,
    debug: false,
    ...
    try {
      const browser = await retry(async () => {
        if (options.debug) {
        ...
        if (pages.length === 0) {
          throw new PlaywrightConnectionError('Browser connected but no pages available');
        }
      ...
      }, options.maxRetries, Math.floor(options.timeoutMs / options.maxRetries));

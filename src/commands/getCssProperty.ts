export default async function getCssProperty(this: any, elementId: string, propertyName: string) {
  ...
  try {
    ...
    const value = await element.evaluate((el: any, propName: string, camelPropName: string) => {
      const styles = window.getComputedStyle(el);
      return styles[camelPropName] || styles.getPropertyValue(propName) || null;
    }, propertyName, camelCasePropertyName);
    
    return value;
  } catch (err: any) {
    throw new Error(`Failed to get CSS property '${propertyName}': ${err.message}`);
  }
} 